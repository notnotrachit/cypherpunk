...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "storeTransaction") {
    storeTransactionInExtension(request.transaction)
      .then(sendResponse)
      .catch((error) => sendResponse({ error: error.message }));
    return true;
  }
...
async function buildTransaction(recipientWallet, amount, senderWallet) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/tokens/build-transaction`,
      {
        method: "POST",
        headers: {
...
async function buildUnlinkedTransaction(socialHandle, amount, senderWallet) {
  ...
  try {
    console.log("Building unlinked transaction for:", {
      senderWallet,
      socialHandle,
      amount: amountInSmallestUnit,
    });
    ...
    // Build transaction via API
    const response = await fetch(
      `${API_BASE_URL}/api/tokens/build-unlinked-transaction`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function storeTransactionInExtension(transaction) {
  try {
    console.log("📡 Posting transaction to API:", transaction.id);

    const response = await fetch(`${API_BASE_URL}/api/transactions/store`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const data = await response.json();
    console.log("✅ Transaction stored via API:", transaction.id);

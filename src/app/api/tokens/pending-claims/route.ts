...
export async function GET(request: NextRequest) {
  try {
    ...
    // Parse: owner (32 bytes), twitter (4 + string) — only considering Twitter here
    ...
    offset += 4;
    const twitter = data
      .slice(offset, offset + twitterLength)
      .toString("utf-8");
    offset += twitterLength;

    const handles = [twitter].filter((h) => h && h.length > 0);
      ...
      if (accountInfo) {
        try {
          ...
          if (handleLength > 30 || handleLength < 1) {
            console.error(
              "PENDING-CLAIMS: Invalid handle length:",
              handleLength,
            );
            console.error(
              "PENDING-CLAIMS: This is likely an old account format created before the contract upgrade",
            );
            console.error(
              "PENDING-CLAIMS: Account needs to be migrated or closed",
            );

            // Skip old accounts silently for now
            console.log(
              "PENDING-CLAIMS: Skipping old format account for handle:",
              handle,
            );
            continue;
          }
          ...
          if (amountOffset + 17 > claimData.length) {
            console.error(
              "PENDING-CLAIMS: Account data too short for new structure, might be old format - skipping",
            );
            continue;
          }
          ...
          const paymentCount = Number(
            claimData.readBigUInt64LE(amountOffset + 9),
          );

          console.log(
            "PENDING-CLAIMS: Found claim for",
            socialHandle,
            "Amount:",
            amount,
            "Claimed:",
          ...
          if (!claimed) {
            ...
            if (paymentCount > 0) {
              try {
                ...
                paymentIndexBuffer.writeBigUInt64LE(
                  BigInt(lastPaymentIndex),
                  0,
                );
                ...
                const paymentRecordInfo =
                  await connection.getAccountInfo(paymentRecordPDA);
                if (paymentRecordInfo) {
                  const sender = new PublicKey(
                    paymentRecordInfo.data.slice(8, 40),
                  );
          ...
          console.error("PENDING-CLAIMS: Error parsing claim data:", msg);
          console.error(
            "PENDING-CLAIMS: This might be an old account format - skipping",
          );
          continue;
        }
      }
    ...
    return NextResponse.json({
      ...
      message:
        claims.length > 0
          ? `Found ${claims.length} pending claim(s)`
          : "No pending claims",
    });
  } catch (error: unknown) {
    console.error("PENDING-CLAIMS: Error:", error);
    const message =
      error instanceof Error ? error.message : "Failed to check pending claims";
